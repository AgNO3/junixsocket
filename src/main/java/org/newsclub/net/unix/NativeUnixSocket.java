...
final class NativeUnixSocket {
    ...
    static void load () {
        if ( !loaded ) {
            ...
            final String libDir = System.getProperty(PROP_LIBRARY_DIR, NativeUnixSocketConfig.LIBRARY_PATH);

            String[] libDirs = libDir == null ? new String[] {
                null
            } : new String[] {
                libDir, null
            };
            String[] javaSpecs = "1.5".equals(javaSpec) ? new String[] {
                javaSpec
            } : new String[] {
                javaSpec, "1.5"
            };
                ...
                for ( String js : javaSpecs ) {
                    ...
                    try {
                        if ( ld == null ) {
                            ...
                            final File libFile = new File(new File(ld), prefix + libId + suffix);
            ...
            if ( ule != null ) {
                throw (UnsatisfiedLinkError) new UnsatisfiedLinkError("Could not load junixsocket library, tried " + paths
                        + "; please define system property " + PROP_LIBRARY_DIR).initCause(ule);
            }
            loaded = true;
        }
    }
    ...
    native static void bind ( final String socketFile, final FileDescriptor fd, final int backlog, boolean abstr ) throws IOException;
    ...
    native static void connect ( final String socketFile, final FileDescriptor fd, boolean abstr ) throws IOException;
    ...
    native static void setSocketOptionInt ( final FileDescriptor fd, int optionId, int value ) throws IOException;


    native static void sendCredentials ( final FileDescriptor fd, byte data ) throws IOException;


    native static int[] receiveCredentials ( final FileDescriptor fd ) throws IOException;
    ...
    native static void initServerImpl ( final AFUNIXServerSocket serverSocket, final AFUNIXSocketImpl impl );
